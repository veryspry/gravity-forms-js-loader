 ...
 */
class GF_Custom_JS_File_Loader {
	...
	public function __construct( $configs = array() ) {
        if ( empty( $configs ) ) {
            throw new Error('GF_Custom_JS_File_Loader: You must pass an array of arguments to the constructor.');
        }

        $this->script_configs = $configs;
	...
	public function register_init_script( $form ) {
        // filter $this->script_configs to only include the ones that have a key ID that is equal to $form['id'];
        $scripts = array_filter( $this->script_configs, function( $script_config ) use ( $form ) {
            return $script_config['form_id'] == $form['id'];
        } );

        // iterate over scripts and add them to the init scripts
        foreach ( $scripts as $script ) {
            // build a path to a directory named "js" that is on the same level as this file.
            $path = dirname( __FILE__ ) . '/js/' . $script['file_name'];
		    ...
		    $script = html_entity_decode(
                str_replace(
                    array_keys( $allowed_entities ), $allowed_entities, $file_contents
                )
            );
		    $script = str_replace( 'GFFORMID', $form['id'], $script );
		    $script = '( function( $ ) { ' . $script . ' } )( jQuery );';

            // hash the file path to create a unique slug
            $hash = md5( $path );
		    $slug = "gf_custom_js_loader_{$form['id']}_{$hash}";
